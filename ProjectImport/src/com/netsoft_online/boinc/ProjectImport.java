...
public class ProjectImport {
	...
	static Logger log = Logger.getLogger(ProjectImport.class.getName()); // log4j
																			// stuff
	...
	public void RunImport() {
		...
		boolean addMissingTypes = cr.GetValueBoolean("import",
				"update_types_tables", false);
		boolean backupFiles = cr.GetValueBoolean("import",
				"backup_files_after_import", false);
		boolean forceImport = cr.GetValueBoolean("import", "read_all_if_one",
				false);
		String lockfile = cr.GetValueString("lockfile", "name",
				"/home/drews/boinc/boinc.lck");
		int lockRetries = cr.GetValueInt("lockfile", "retries", 5);
		int lockRetryWait = cr.GetValueInt("lockfile", "retry_wait", 1);
		String dbTempFile = cr.GetValueString("database", "tempfile",
				"/tmp/import.tmp");

		myDB.SetDatabaseInfo(database_driver, database_name, database_user,
				database_pass);
				...
				try {
					...
					if (lock == null) {
						...
						try {
							...
							Thread.sleep(lockRetryWait * 60 * 1000); // Try to
																		// sleep
																		// for 5
																		// min
						} catch (Exception se) {
			...
			if (cr.GetValueBoolean("import", "read_files", true)) {
				try {
					...
					for (int i = 0; i < myProjects.size(); i++) {
						...
						// For each project, if the file exists, compare the
						// time stamp
						...
						// -if all goes well, drop production table, rename
						...
						// NOTE: bcs_id could be dangerous - wcg just had a
						// duplicate cpid show
						...
						try {
							...
							String filename = p.data_dir + p.shortname
									+ PLATFORM_DIR_CHAR + p.team_file;
							...
							String lastImportTime = myDB
									.ProjectGetTeamUpdateTime(p.project_id);
							if (lastImportTime == null
									|| currentFileTime.substring(0, 16)
											.compareTo(
													lastImportTime.substring(0,
															16)) != 0) {
								teamImportNeeded = true;
							}
							// Check time of user file
							filename = p.data_dir + p.shortname
									+ PLATFORM_DIR_CHAR + p.user_file;
							f = new File(filename);
							...
							lastImportTime = myDB
									.ProjectGetUserUpdateTime(p.project_id);
							if (lastImportTime == null
									|| currentFileTime.substring(0, 16)
											.compareTo(
													lastImportTime.substring(0,
							...
							currentFileTime = sdf.format(when);
							lastImportTime = myDB
									.ProjectGetHostUpdateTime(p.project_id);
							if (lastImportTime == null
									|| currentFileTime.substring(0, 16)
											.compareTo(
													lastImportTime.substring(0,
							...
							if (forceImport == true
									&& (hostImportNeeded == true
											|| userImportNeeded == true || teamImportNeeded == true)) {
								hostImportNeeded = true;
								userImportNeeded = true;
								teamImportNeeded = true;
							}
							...
							if (teamImportNeeded == true
									|| userImportNeeded == true
									|| hostImportNeeded == true) {
								log.info("Starting project " + p.name
										+ " data import (" + i + " of "
										+ myProjects.size() + ")");

								hTeams.clear();
								try {
									myDB.TeamRead(hTeams, "teams_"
											+ p.shortname);
									log.info("Read in " + hTeams.size()
											+ " teams from project " + p.name);
								} catch (java.sql.SQLException ss) {
									// table may not exist if project is new
									log.error("Failed to read from table teams_"
											+ p.shortname);
									try {
										myDB.CreateTeamsTable(p.shortname);
									} catch (java.sql.SQLException ss2) {
										log.error("Failed to create table teams_"
												+ p.shortname);
									}
								}
							}
							// if the team file exists, blast it into the backup
							// table
							if (p.team_file != null && teamImportNeeded == true) {

								// drop team backup table (may not exist)
								try {
									myDB.DropTable("teams_" + p.shortname
											+ "_bk");
								} catch (java.sql.SQLException s) {
									// ignoring this, table probably doesn't
									// exist (it shouldn't)
								}
								...
								try {
									...
									filename = p.data_dir + p.shortname
											+ PLATFORM_DIR_CHAR + p.team_file;
									updateTeamCount = ReadProjectTeamXMLFile(
											filename, "teams_" + p.shortname
													+ "_bk", dbTempFile);
									...
									if (updateTeamCount < 0) {
										...
										try {
											...
											myDB.DoProjectTeamRankings("teams_"
													+ p.shortname + "_bk");

											// Add Indexes
											log.debug("Adding table indexes");
											String query = "ALTER TABLE teams_"
													+ p.shortname
													+ "_bk  ADD INDEX `project_rank_credit`(`project_rank_credit`), ADD INDEX `project_rank_rac`(`project_rank_rac`)";
											myDB.RunQuery(query);

										} catch (java.sql.SQLException ss) {
											log.error(
													"Failed to swap team tables",
													ss);
										}
										...
										myDB.ProjectUpdateTeamsTime(
												p.project_id, lastImport);
										teamImported = true;
									}
								} catch (java.sql.SQLException sqle) {
									log.error("Failed processing TEAM file",
											sqle);
								} catch (Exception e) {
							...
							if (userImportNeeded == true
								...
								hUsers.clear();
								try {
									myDB.UserRead(hUsers, "users_"
											+ p.shortname);
									log.info("Read in " + hUsers.size()
											+ " users from project " + p.name);
								} catch (java.sql.SQLException ss) {
									// table may not exist if project is new
									log.error("Failed to read from table users_"
											+ p.shortname);
									try {
										myDB.CreateUsersTable(p.shortname);
									} catch (java.sql.SQLException ss2) {
										log.error("Failed to create table users_"
												+ p.shortname);
									}
								}
							}
							...
							if (p.user_file != null && userImportNeeded == true) {
								...
								try {
									myDB.DropTable("users_" + p.shortname
											+ "_bk");
								} catch (java.sql.SQLException s) {
									// ignoring this, table probably doesn't
									// exist (it shouldn't)
								...
								filename = p.data_dir + p.shortname
										+ PLATFORM_DIR_CHAR + p.user_file;
								updateUserCount = ReadProjectUserXMLFile(
										filename, "users_" + p.shortname
												+ "_bk", p.project_id,
										dbTempFile);

								if (updateUserCount < 0)
									log.error("Failed to import data from file "
											+ filename + " - file incomplete");
								else {

									try {
										// Update Rankings
										log.debug("Updating User Rankings");
										myDB.DoProjectUserRankings("users_"
												+ p.shortname + "_bk");

										// Update user counts in teams table
										if (teamImported == true) {
											log.debug("Updating user counts for teams");
											myDB.DoUpdateTeamUserCounts(
													"users_" + p.shortname
															+ "_bk", "teams_"
															+ p.shortname
															+ "_bk");
										...
										String query = "ALTER TABLE users_"
												+ p.shortname
												+ "_bk ADD INDEX `user_cpid`(`user_cpid`), ADD INDEX `b_cpid_id`(`b_cpid_id`), ADD INDEX `b_cteam_id`(`b_cteam_id`), ADD INDEX `project_rank_credit`(`project_rank_credit`), ADD INDEX `project_rank_rac`(`project_rank_rac`)";
										myDB.RunQuery(query);

									} catch (java.sql.SQLException ss) {
										log.error("Failed to swap user tables",
												ss);
									}
									...
									sdf.setTimeZone(TimeZone.getDefault());
									String lastImport = sdf.format(when);
									myDB.ProjectUpdateUsersTime(p.project_id,
											lastImport);
									userImported = true;
							...
							// Since we are not tracking history, we don't need
							// to cache
							// this set...
							...
							if (p.host_file != null && hostImportNeeded == true) {
								...
								// create backup table
								try {
									myDB.DropTable("hosts_" + p.shortname
											+ "_bk");
								} catch (java.sql.SQLException ss2) {
								...
								try {
									...
									log.error("Failed to create backup table hosts_"
											+ p.shortname + "_bk");
								}
								...
								try {
									filename = p.data_dir + p.shortname
											+ PLATFORM_DIR_CHAR + p.host_file;
									updateHostCount = ReadProjectHostXMLFile(
											filename, p.project_id, "hosts_"
													+ p.shortname + "_bk",
											addMissingTypes, dbTempFile);
								} catch (Exception e) {
									log.error("Error", e);
								}
								if (updateHostCount < 0) {
									...
									try {
										...
										if (cr.GetValueBoolean("import",
												"rank_hosts", false) == true) {
											// Update Rankings
											log.debug("Updating Host Rankings");
											myDB.DoProjectHostRankings("hosts_"
													+ p.shortname + "_bk");
										}
										...
										// log.debug("Adding indexes to table");
										// String query =
										// "ALTER TABLE hosts_"+p.shortname+"_bk ADD INDEX `host_cpid`(`host_cpid`), ADD INDEX `b_cpid_id`(`b_cpid_id`), ADD INDEX `user_id`(`user_id`), ADD INDEX `project_rank_credit`(`project_rank_credit`), ADD INDEX `project_rank_rac`(`project_rank_rac`)";
										// myDB.RunQuery(query);
										// log.debug("Done adding indexes");

									} catch (java.sql.SQLException ss) {
										log.error("Failed to swap host tables",
												ss);
									}
									...
									sdf.setTimeZone(TimeZone.getDefault());
									String lastImport = sdf.format(when);
									myDB.ProjectUpdateHostsTime(p.project_id,
											lastImport);
									hostImported = true;
								}
							}
							...
							try {
								if (teamImportNeeded == true
									...
									myDB.RenameTable("teams_" + p.shortname,
											"teams_" + p.shortname + "_old");
									// rename backup to production
									myDB.RenameTable("teams_" + p.shortname
											+ "_bk", "teams_" + p.shortname);
									// drop old
									myDB.DropTable("teams_" + p.shortname
											+ "_old");
								} else {
								...
								if (userImportNeeded == true
									...
									myDB.RenameTable("users_" + p.shortname,
											"users_" + p.shortname + "_old");
									// rename backup to production
									myDB.RenameTable("users_" + p.shortname
											+ "_bk", "users_" + p.shortname);
									// drop old
									myDB.DropTable("users_" + p.shortname
											+ "_old");
								} else {
								...
								if (hostImportNeeded == true
									...
									// rename production table to old
									myDB.RenameTable("hosts_" + p.shortname,
											"hosts_" + p.shortname + "_old");
									// rename backup to production
									myDB.RenameTable("hosts_" + p.shortname
											+ "_bk", "hosts_" + p.shortname);
									// drop old
									myDB.DropTable("hosts_" + p.shortname
											+ "_old");
								} else {
								...
								log.error(
										"Failed to move tables to production",
										se);
							}
							...
							if (teamImportNeeded == true
								...
								try {
									...
									myDB.DoProjectStats(p.project_id, "users_"
											+ p.shortname, "hosts_"
											+ p.shortname, "teams_"
											+ p.shortname);
								} catch (java.sql.SQLException se) {
									log.error(
											"Failed to update project table stats",
											se);
								}
								log.info("Importing files done " + p.name
										+ " data import");
							}

							// create a backup of the files for each one that
							// was good
							String backup_file;
							if (backupFiles == true && userImportNeeded == true
									&& updateUserCount >= 0) {
								filename = p.data_dir + p.shortname
										+ PLATFORM_DIR_CHAR + p.user_file;
								backup_file = p.data_dir + p.shortname
										+ PLATFORM_DIR_CHAR + p.shortname + "_"
										+ p.user_file;
								log.debug("Creating backup file " + backup_file);
								BackupFile(filename, backup_file);
							}
							if (backupFiles == true && hostImportNeeded == true
									&& updateHostCount >= 0) {
								filename = p.data_dir + p.shortname
										+ PLATFORM_DIR_CHAR + p.host_file;
								backup_file = p.data_dir + p.shortname
							...
							if (backupFiles == true && teamImportNeeded == true
									&& updateTeamCount >= 0) {
								filename = p.data_dir + p.shortname
										+ PLATFORM_DIR_CHAR + p.team_file;
								backup_file = p.data_dir + p.shortname
	...
	public int ReadProjectUserXMLFile(String filename, String tablename,
			int project_id, String tempfile) {
		...
		try {
			...
			org.xml.sax.ContentHandler handler = new UserXMLFileHandler(
					tablename, project_id, hUsers, hCountries, hTeams, hCpid,
					myDB, racTime, tempfile);
			...
			myDB.RunQuery("LOAD DATA LOCAL INFILE '" + tempfile
	...
	public int ReadProjectHostXMLFile(String filename, int project_id,
			String tablename, boolean addMissingTypes, String tempfile) {
		...
		try {
			...
			XMLReader parser = XMLReaderFactory
					.createXMLReader("org.apache.xerces.parsers.SAXParser");
			org.xml.sax.ContentHandler handler = new HostXMLFileHandler(hUsers,
					hPvendor, hPmodel, hOsname, myDB, racTime, tablename,
					addMissingTypes, tempfile);
			...
			log.info("Added " + added + " to hosts to table " + tablename
					+ " (" + anonymous + " anonymous)");
	...
	public int ReadProjectTeamXMLFile(String filename, String tablename,
			String tempfile) {
		...
		try {
			...
			XMLReader parser = XMLReaderFactory
					.createXMLReader("org.apache.xerces.parsers.SAXParser");
			org.xml.sax.ContentHandler handler = new TeamXMLFileHandler(
					tablename, hTeams, hCountries, hCTeam, myDB, racTime,
					tempfile);
	...
	public void BackupFile(String from_name, String to_name) {
		...
		try {
			...
			File from_file = new File(from_name); // Get File objects from
													// Strings
			...
			if (to_file.exists()) {
				...
				// current dir.
				// so look up the user.dir system property to find out what that
				// is.
