...
if (! function_exists ( "statsdb_connect" )) {
	function statsdb_connect() {
...
if (! function_exists ( "project_color" )) {
	function project_color($p) {
		...
		$query = "select project_color from projects where name = '$p'";
		
		$res = mysqli_query ( $dbhandle, $query );
		if (! $res) {
			return "DB Error $dbname ";
		} else {
...
if (! function_exists ( "display_project_stats" )) {
	function display_project_stats($active) {
		...
		if ($active > 0) {
			$query = "select a.name,a.url,a.user_count,a.host_count,a.team_count," . "a.country_count,a.total_credit,a.project_id,a.y_user_count,a.z_user_count," . "a.y_host_count,a.y_team_count,a.y_country_count,a.y_total_credit," . "a.z_host_count,a.z_team_count,a.z_country_count,a.z_total_credit," . "a.last_update_users,a.active_hosts,a.active_users,a.active_teams," . "a.z_active_users,a.z_active_teams,a.z_active_hosts," . "a.y_active_users,a.y_active_teams,a.y_active_hosts,a.shown, a.retired from projects a " . "where (a.retired='N' and a.shown='Y') or a.project_id=1 order by a.total_credit desc";
		} else {
			$query = "select a.name,a.url,a.user_count,a.host_count,a.team_count," . "a.country_count,a.total_credit,a.project_id,a.y_user_count,a.z_user_count," . "a.y_host_count,a.y_team_count,a.y_country_count,a.y_total_credit," . "a.z_host_count,a.z_team_count,a.z_country_count,a.z_total_credit," . "a.last_update_users,a.active_hosts,a.active_users,a.active_teams," . "a.z_active_users,a.z_active_teams,a.z_active_hosts," . "a.y_active_users,a.y_active_teams,a.y_active_hosts, a.shown, a.retired from projects a " . "where a.retired='Y' and a.shown='Y' order by a.total_credit desc";
		}
		$res = mysqli_query ($dbhandle, $query );
		if (! $res) {
			$DATA = "<b>Error performing query: " . mysqli_error ($dbhandle) . "</b>";
			return $DATA;
		}
     ...
     <th colspan=1 class='forumheader'><font size =-1>" . LAN_322 . "</font></th>
		...
		while ( ($row = mysqli_fetch_array ( $res )) ) {
			...
			$zateam_count = $row ["z_active_teams"];
			$shown = $row ["shown"];
			$retired = $row ["retired"];
			...
			if ($active > 0)
				$DATA .= "<td align=right class='$bc'><font size=-1 color=\"$cuc\">($suc$uc)</font></td>\n";
			$DATA .= "<td align=right class='$bc'>$host_count</td>\n";
			if ($active > 0)
				$DATA .= "<td align=right class='$bc'><font size=-1 color=\"$chc\">($shc$hc)</font></td>\n";
			$DATA .= "<td align=right class='$bc'>$team_count</td>\n";
			if ($active > 0)
				$DATA .= "<td align=right class='$bc'><font size=-1 color=\"$ctc\">($stc$tc)</font></td>\n";
			if ($active > 0)
...
if (! function_exists ( "display_users_ranking" )) {
	function display_users_ranking($table, $start, $end, $field, $opt1, $opt2, $highlight, $tc) {
		...
		$query = "select a.user_cpid,a.name,a.project_count,a.active_project_count,a.total_credit,a.rac,a.$field,country.country " . "from $table a left join country " . "on a.country_id=country.country_id " . "where $field >= $start and $field < $end";
		...
		$res = mysqli_query ($dbhandle, $query );
		if (! $res) {
			return "<b>Invalid Rank Requested</b>";
		}
